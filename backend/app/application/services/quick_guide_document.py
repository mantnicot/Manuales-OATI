...
def quick_guide_html(manual: Manual) -> str:
    ...
    if not pages:
        ...
        sheets = "".join(_render_page(dict(p.data), idx + 1, len(pages)) for idx, p in enumerate(pages))
...
def _render_page(data: dict, page_num: int, total_pages: int) -> str:
    ...
    foot = (
        f"<footer class='qg-foot'><span>OATI — Oficina Asesora de Tecnologías de la Información</span>"
        f"<span>Página {page_num} / {total_pages}</span></footer>"
    )
...
.qg-theme-glossary .qg-body {
  background: repeating-linear-gradient(90deg, #f0f0f2 0 13mm, #e0e0e6 13mm 26mm);
}
.qg-theme-guide .qg-body {
  background: repeating-linear-gradient(90deg, #f8e8ec 0 15mm, #ece4f0 15mm 30mm);
}
